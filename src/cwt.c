...
static void wave_function(int nk, double dt,int mother, double param,double scale1, double *kwave, double pi,double *period1,
	...
	if (mother == 0) {
		...
		*coi1 = fourier_factor * sqrt(2.0);
		...
		norm = sqrt(2.0*pi*scale1 / dt)*sqrt(1.0 / _gamma(m + 0.50));
		norm *= sign;
	...
	if (mother == 0) {
		...
		*real = 1;
		...
		if (m % 2 == 0) {
			...
			coeff = sign * pow(2.0, (double)m / 2) / _gamma(0.5);
			*val = coeff * _gamma(((double)m + 1.0) / 2.0) / sqrt(_gamma(m + 0.50));
		}
		else {
			*val = 0;
		}
...
double cdelta(int mother, double param, double psi0 ) {
	...
	else if (mother == 2)
	{
		...
		printf("Mother is neither 0, 1, nor 2.\n");
		return 0.0;
	}
